...
const translations = {
    en: {
        ...
        'deploy.p2_desc': 'Deploy Segment SID — encode 30s snapshots as discrete tokens. Constrained decoding generates only valid (online) segments. High-light PRM scores each moment for recommendation timing.',
        ...
        'deploy.p4_title': 'Sustainable Ecosystem Generation',
        'deploy.p4_problem': 'Problem: Optimizing one metric hurts others. Gift-optimized generation starves new creators; engagement-only generation kills revenue.',
        'deploy.p4_desc': 'Multi-stakeholder generation with governance constraints. Creator fairness as first-class generation constraint, not post-hoc reranking. Pareto-optimal balance across user, platform, and creator objectives.',
        'deploy.p4_adds': 'Unlocks: Generation serves all stakeholders — ecosystem-level sustainability',
        // Footer
    ...
    zh: {
        ...
        'deploy.p2_desc': '部署片段SID——将30秒快照编码为离散token。约束解码仅生成有效（在线）片段。高光PRM为每个时刻打分以优化推荐时机。',
        ...
        'deploy.p4_title': '可持续生态生成',
        'deploy.p4_problem': '问题：优化单一指标损害其他指标。礼物优化导致新创作者饥饿；只优化参与度扼杀收入。',
        'deploy.p4_desc': '带治理约束的多方利益生成。创作者公平性作为第一等生成约束，而非事后重排。用户、平台、创作者目标的Pareto最优平衡。',
        'deploy.p4_adds': '解锁：生成服务所有利益方——生态级可持续性',
        // Footer
        'footer.title': '可控推荐生成',
