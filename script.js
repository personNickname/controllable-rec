...
const translations = {
    en: {
        ...
        'overview.live_heading': 'Why Livestream Is the Hardest Case',
        'overview.property': 'Property',
        'overview.static_rec': 'Static Rec',
        'overview.live_rec': 'Livestream',
        ...
        'overview.engage_s': 'Single click/purchase',
        'overview.engage_l': '10min+ continuous viewing',
        'overview.realtime': 'Real-time Feedback',
        'overview.realtime_s': 'Post-consumption rating',
        'overview.realtime_l': 'Comments, gifts arrive during consumption',
        'overview.creator': 'Creator Ecosystem',
        'overview.creator_s': 'Supply stable',
        'overview.creator_l': 'Streamer cold-start, exposure fairness',
        ...
        // Thesis — Livestream
        'thesis.scene_title': 'Industry Scene: Entertainment Livestream',
        'thesis.why_live': 'Why Livestream?',
        'thesis.why_live_desc': 'Livestream is the hardest controllability testbed — content changes every second, feedback is real-time, and three stakeholders (user, streamer, platform) compete for every recommendation slot. If controllability works here, it works anywhere.',
        ...
        'ls.user_desc': 'Gift propensity, entertainment mood, watch duration intent. Gift history provides a much stronger preference signal than clicks — a user who gifts ¥100 to a singer has expressed far more intent than one who clicked.',
        'ls.item_title': '🎬 In Livestream',
        'ls.item_desc': 'A livestream is not one item — it\'s a sequence of evolving 30s micro-items (Foresight Semantic ID segments). Controlling "when to recommend" (during high-light moments) is where real value lies, directly boosting both consume value and gift value.',
        'ls.biz_title': '🎬 In Livestream',
        'ls.biz_desc': 'Optimize α×consume_value + β×gift_value. Streamer ecosystem is more fragile than product catalogs — a streamer who gets no exposure will quit (unlike a product that stays in inventory). Cold-start fairness is a first-class business objective.',
        'ls.timing_title': '🎬 In Livestream',
        'ls.timing_desc': 'Mid-generation timing is unique to livestream. User is watching → system must decide WHEN to surface the next stream. Interrupting during high-light hurts gift value; waiting during lull loses the user.',
        'ls.gran_title': '🎬 In Livestream',
        'ls.gran_desc': 'Foresight\'s 30s segment Semantic ID enables fine-grained control — distinguish "this streamer right now" from "5 minutes ago." Three levels: genre (coarse) → streamer style (medium) → this-30s-moment (fine).',
        'ls.expl_title': '🎬 In Livestream',
        'ls.expl_desc': 'Temporal explanations become possible: "why now" is as important as "why this streamer." Examples: "This streamer is in a high-energy moment" / "500 viewers are gifting right now" (social proof).',
        'ls.fb_title': '🎬 In Livestream',
        'ls.fb_desc': 'Feedback is continuous and real-time, not post-hoc. A gift mid-stream is both a business metric (revenue) AND a feedback signal (user likes this). This creates a closed loop: feedback → update recommendation of the SAME stream to other users.',
        'ls.mech_live': '🎬 Livestream:',
        'ls.cond_live': 'Segment Semantic ID (Foresight), streamer profile tokens, real-time state embedding (energy/topic/interaction)',
        'ls.pref_live': 'Gift-based DPO (gifted > non-gifted), Hierarchical DPO (大额 > 小额 > 长看 > 评论 > 跳过), joint α×L_gift + β×L_consume training',
        'ls.dec_live': 'High-light PRM (LiveForesighter moment detection), state-aware beam search weighted by live/energy/interaction, crowd gift/comment rate as beam score',
        'ls.post_live': 'Multi-signal reranking (relevance + gift propensity + consume value + fairness), real-time state refresh (drop ended streams, boost high-light, penalize lull), streamer exposure equity',
        // Industry
    ...
    zh: {
        ...
        'hero.gift': '礼物价值',
        'hero.consume': '消费价值',
        'hero.tagline': '生成式推荐系统的3轴控制分类体系，扎根于娱乐直播推荐场景',
        // Overview
        'overview.title': '概述',
        ...
        // Thesis — Livestream
        'thesis.scene_title': '工业场景：娱乐直播',
        'thesis.why_live': '为什么是直播？',
        'thesis.why_live_desc': '直播是最难的可控性试验场——内容逐秒变化，反馈实时到达，三方利益相关者（用户、主播、平台）争夺每个推荐位。如果可控性在这里能行，在任何场景都能行。',
        'thesis.metrics': '核心指标',
        'thesis.metric': '指标',
        'thesis.def': '定义',
        'thesis.role': '角色',
        ...
        // Livestream mappings
        'ls.user_title': '🎬 直播场景',
        'ls.user_desc': '打赏倾向、娱乐情绪、观看时长意图。打赏历史提供远强于点击的偏好信号——向歌手打赏¥100的用户比仅点击的用户表达了更多意图。',
        'ls.item_title': '🎬 直播场景',
        'ls.item_desc': '直播不是一个物品——它是一连串不断演变的30秒微物品（Foresight Semantic ID片段）。控制"何时推荐"（在高光时刻）才是价值所在，直接提升消费价值和礼物价值。',
        'ls.biz_title': '🎬 直播场景',
        'ls.biz_desc': '优化 α×消费价值 + β×礼物价值。主播生态比商品目录更脆弱——得不到曝光的主播会离开（不像商品留在库存中）。冷启动公平性是一等业务目标。',
        'ls.timing_title': '🎬 直播场景',
        'ls.timing_desc': '生成中时序是直播独有的。用户正在观看 → 系统必须决定何时推荐下一个直播间。在高光时刻打断损害礼物价值；在低谷等待过久失去用户。',
        'ls.gran_title': '🎬 直播场景',
        'ls.gran_desc': 'Foresight的30秒片段Semantic ID实现细粒度控制——区分"这个主播现在"和"5分钟前"。三层：类型（粗）→ 主播风格（中）→ 这30秒时刻（细）。',
        'ls.expl_title': '🎬 直播场景',
        'ls.expl_desc': '时间维度的解释成为可能："为什么是现在"和"为什么是这个主播"同样重要。例如："这个主播正处于高能时刻" / "500位观众正在打赏"（社交证明）。',
        'ls.fb_title': '🎬 直播场景',
        'ls.fb_desc': '反馈是连续实时的，而非事后的。观看中的打赏既是业务指标（收入）又是反馈信号（用户喜欢这个）。这创造了闭环：反馈 → 更新对同一直播间向其他用户的推荐。',
        'ls.mech_live': '🎬 直播：',
        'ls.cond_live': '片段Semantic ID（Foresight）、主播画像令牌、实时状态嵌入（能量/话题/互动强度）',
        'ls.pref_live': '礼物DPO（打赏 > 未打赏）、层次化DPO（大额 > 小额 > 长看 > 评论 > 跳过）、联合 α×L_gift + β×L_consume 训练',
        'ls.dec_live': '高光PRM（LiveForesighter时刻检测）、状态感知Beam搜索（按直播/能量/互动加权）、人群打赏/评论率作为beam分数',
        'ls.post_live': '多信号重排序（相关性+打赏倾向+消费价值+公平性）、实时状态刷新（下线降权、高光提权、低谷降权）、主播曝光公平',
        // Industry
        'nav.industry': '工业实践',
