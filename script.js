...
const translations = {
    en: {
        ...
        'thesis.why_ctrl': 'Why Controllability — Not Generation Quality',
        'thesis.why_ctrl_desc': 'In static rec, uncontrolled generation still produces valid items — they exist in the catalog. In livestream, uncontrolled generation is fundamentally broken: it recommends offline liverooms (invalid), boring moments of good streamers (mistimed), and ignores that a user who loves gaming clips might love gaming livestreams (no cross-domain transfer). This isn\'t a quality problem — it\'s a correctness problem. Without controllability, livestream gen-rec can\'t even be deployed.',
        ...
        'thesis.moat_desc': 'Generation is already commoditized — TIGER, GPR, OxygenREC all achieve comparable generation quality. But our paper mapping shows Post-Processing control has only 3 papers, Business×Decoding has 2, and no paper addresses cross-domain interest transfer for livestream. The moat is in the controllability stack: whoever builds verifiable constraints, calibrated multi-objective tradeoffs, and real-time feedback loops first owns the deployment gap.',
    ...
    zh: {
        ...
        'thesis.why_ctrl': '为什么是可控性——而非生成质量',
        'thesis.why_ctrl_desc': '在静态推荐中，不可控的生成至少产生有效物品——它们存在于目录中。在直播中，不可控的生成从根本上就是坏的：推荐已下播的直播间（无效）、推荐好主播的无聊时段（错时）、忽略一个爱看游戏短视频的用户也可能爱看游戏直播（无跨域迁移）。这不是质量问题——是正确性问题。没有可控性，直播生成式推荐根本无法上线。',
        'thesis.diff': '与LLM对齐的区别',
        'thesis.diff_desc': 'LLM对齐是单用户生成。推荐可控性是多利益主体约束搜索——用户、平台、创作者利益冲突。RLHF/Constitutional AI 不直接适用。',
        'thesis.moat': '壁垒在哪里',
        'thesis.moat_desc': '生成能力已被商品化——TIGER、GPR、OxygenREC都达到了相当的生成质量。但论文映射显示：后处理控制仅3篇、业务×解码仅2篇、直播跨域兴趣迁移0篇。壁垒在可控性技术栈：谁先建成可验证约束、可校准多目标权衡、实时反馈闭环，谁就占据部署缺口。',
